...
if (!process.env.SITE_URL || !process.env.NODE_ENV || !process.env.PORT) {
  process.stderr.write("You must provide NODE_ENV, PORT and SITE_URL environments variables.");
  process.exit(1);
}
...
gulp.task('html', () => {
  return gulp.src('source/pages/**')
    ...
    .pipe(gulp.dest('build'))
    .pipe(sitemap({ siteUrl: process.env.SITE_URL }))
    .pipe(gulp.dest('./build'))
    .pipe(connect.reload());
});
